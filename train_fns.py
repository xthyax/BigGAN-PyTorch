...
def GAN_training_function(G, D, GD, z_, y_, ema, state_dict, config):
	def train(x, y):
		...
		x = torch.split(x, min(config['batch_size'], int(current_batch/ (config['num_D_steps'] * config['num_D_accumulations']))))
		y = torch.split(y, min(config['batch_size'], int(current_batch/ (config['num_D_steps'] * config['num_D_accumulations']))))
		counter = 0
		# print(len(x))
...
''' This function runs the inception metrics code, checks if the results
    ...
    improvement. '''
def test(G, D, G_ema, z_, y_, state_dict, config, sample, get_inception_metrics, test_log):
	...
	if ((config['which_best'] == 'IS' and IS_mean > state_dict['best_IS'])
		...
		utils.save_weights(G, D, state_dict, config['logs_root'], 'best',G_ema if config['ema'] else None)
